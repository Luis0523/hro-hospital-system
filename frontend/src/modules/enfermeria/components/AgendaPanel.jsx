...
export default function AgendaPanel({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      <button
        type="button"
        aria-label="Cerrar panel de agendamiento"
        onClick={onCerrar}
        className="absolute inset-0 bg-on-surface/40"
      />
      <aside className="relative flex h-full w-[min(420px,100%)] flex-col bg-surface-container-lowest shadow-modal">
        <header className="flex items-center justify-between border-b border-outline-variant px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-headline-sm text-on-surface">Agendar cita</h2>
          </div>
          <button
            type="button"
            onClick={onCerrar}
          ...
          {citaCreada ? (
            ...
            <>
              ...
              <div className="space-y-2">
                ...
                {cargandoCupos && (
                  <p className="text-body-sm text-on-surface-variant">Cargando cupos...</p>
                )}
                {!cargandoCupos && cupos.length === 0 && (
                  <p className="rounded-lg bg-surface-container-low px-3 py-3 text-body-sm text-on-surface-variant">
              ...
              {error && (
                <p className="rounded-lg bg-error-container/70 px-3 py-2 text-body-sm text-on-error-container">
                  {error}
                </p>
              )}
            </>
