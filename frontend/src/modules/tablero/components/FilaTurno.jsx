...
export default function FilaTurno({ asignacion }) {
  ...
  return (
    <tr data-testid={`fila-turno-${asignacionDiariaEspacioId}`} className="align-middle">
      ...
      <td className="px-2 py-4 md:px-4 md:py-5">
        <span className="text-[clamp(2.25rem,3.5vw,7rem)] font-bold leading-none tabular-nums text-on-surface">
          {espacioNumero ?? '—'}
        </span>
      </td>

      <td className="px-2 py-4 text-right md:px-4 md:py-5">
        <span className="text-[clamp(3rem,4.5vw,10rem)] font-bold leading-none tabular-nums text-primary">
